		...
		<?php foreach ( $platform['questions'] as $index => $question ) : ?>
			<div style="background:#fff;padding:16px;margin:0 0 12px;border:1px solid #ccd0d4;">
				...
				<p>
					...
					<?php if ( ! empty( $question['compare_left_image_url'] ) ) : ?>
						<br>
						<img src="<?php echo esc_url( $question['compare_left_image_url'] ); ?>" class="cpai-image-preview" />
						<button type="button" class="button cpai-remove-image-btn"><?php esc_html_e( 'Remove Image', 'coachpro-ai-teacher-social-branding' ); ?></button>
					<?php else : ?>
						<img class="cpai-image-preview" style="display:none;" />
						<button type="button" class="button cpai-remove-image-btn" style="display:none;"><?php esc_html_e( 'Remove Image', 'coachpro-ai-teacher-social-branding' ); ?></button>
					<?php endif; ?>
				</p>
				<p>
					...
					<button type="button" class="button cpai-upload-image-btn" style="margin-top:8px;"><?php esc_html_e( 'Upload / Select Image', 'coachpro-ai-teacher-social-branding' ); ?></button>
					<?php if ( ! empty( $question['compare_right_image_url'] ) ) : ?>
						<br>
						<img src="<?php echo esc_url( $question['compare_right_image_url'] ); ?>" class="cpai-image-preview" />
						<button type="button" class="button cpai-remove-image-btn"><?php esc_html_e( 'Remove Image', 'coachpro-ai-teacher-social-branding' ); ?></button>
					<?php else : ?>
						<img class="cpai-image-preview" style="display:none;" />
						<button type="button" class="button cpai-remove-image-btn" style="display:none;"><?php esc_html_e( 'Remove Image', 'coachpro-ai-teacher-social-branding' ); ?></button>
				...
				<p><label><?php esc_html_e( 'Tool Link / Embed (EN HTML)', 'coachpro-ai-teacher-social-branding' ); ?></label><textarea class="widefat" rows="2" name="platform[questions][<?php echo esc_attr( $index ); ?>][instruction_en][tool_link]"><?php echo esc_textarea( isset( $question['instruction_en']['tool_link'] ) ? $question['instruction_en']['tool_link'] : $question['instruction_en']['tool'] ); ?></textarea><span class="description"><?php esc_html_e( 'Paste a full URL to auto-create a clickable link on the frontend, or add custom HTML such as <a href="https://example.com" target="_blank" rel="noopener noreferrer">Open tool</a>.', 'coachpro-ai-teacher-social-branding' ); ?></span></p>
				<p><label><?php esc_html_e( 'Prompt Template (EN)', 'coachpro-ai-teacher-social-branding' ); ?></label><textarea class="widefat" rows="4" name="platform[questions][<?php echo esc_attr( $index ); ?>][instruction_en][prompt_template]"><?php echo esc_textarea( isset( $question['instruction_en']['prompt_template'] ) ? $question['instruction_en']['prompt_template'] : '' ); ?></textarea></p>
				<p><label><?php esc_html_e( 'Tool Link / Embed (UR HTML)', 'coachpro-ai-teacher-social-branding' ); ?></label><textarea class="widefat" rows="2" dir="rtl" name="platform[questions][<?php echo esc_attr( $index ); ?>][instruction_ur][tool_link]"><?php echo esc_textarea( isset( $question['instruction_ur']['tool_link'] ) ? $question['instruction_ur']['tool_link'] : $question['instruction_ur']['tool'] ); ?></textarea><span class="description"><?php esc_html_e( 'یہاں مکمل URL دینے سے فرنٹ اینڈ پر خودکار clickable link بن جائے گا، یا آپ اپنی مرضی کا HTML بھی دے سکتے ہیں جیسے <a href="https://example.com" target="_blank" rel="noopener noreferrer">ٹول کھولیں</a>.', 'coachpro-ai-teacher-social-branding' ); ?></span></p>
				<p><label><?php esc_html_e( 'Prompt Template (UR)', 'coachpro-ai-teacher-social-branding' ); ?></label><textarea class="widefat" rows="4" dir="rtl" name="platform[questions][<?php echo esc_attr( $index ); ?>][instruction_ur][prompt_template]"><?php echo esc_textarea( isset( $question['instruction_ur']['prompt_template'] ) ? $question['instruction_ur']['prompt_template'] : '' ); ?></textarea></p>
			</div>
		<?php endforeach; ?>
